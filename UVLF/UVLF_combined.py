...
def cal_UVLF(magnitude, boxsize, nbins=30, mag_range=[-25, -14],useMask=True):
    """
    ...
    if useMask:
        ...
        logphi = np.full_like(phi, -np.inf, dtype=float)
        logphi_err = np.full_like(phi_err, -np.inf, dtype=float)
        mask = num > 0
        logphi[mask] = np.log10(phi[mask])
        logphi_err[mask] = phi_err[mask] / (phi[mask] * np.log(10))
...
obsInfo =  {"Harikane+ 2022":[str(os.path.join(UVdatadir, "harikane2022_z#.txt")), -1, "purple", harikane_fit_kwargs],
                     "McLeod+ 2023":[str(os.path.join(UVdatadir, "mcleod2023_z11.txt")), 11, "blue", logy_kwargs],
...
for i in range(nred):
    ...
    # storage for dust/no-dust LFs from boxes
    combined_bins = None
    dust_25 = dust_50 = None
    nd_25 = nd_50 = None
    ...
    for box, filelist in boxes.items():
        ...
        # extract magnitudes
        mag_dust = np.array([g.absmag["i1500"] for g in obj.galaxies])
        mag_nodust = np.array([g.absmag_nodust["i1500"] for g in obj.galaxies])

        # apply magnitude cuts
        if box == "25":
            mag_dust = mag_dust[mag_dust < MAGCUT_25]
            mag_nodust = mag_nodust[mag_nodust < MAGCUT_25]
        else:
            mag_dust = mag_dust[mag_dust < MAGCUT_50]
            mag_nodust = mag_nodust[mag_nodust < MAGCUT_50]

        # compute UVLF
        bins, logphi_dust, logphi_dust_err = cal_UVLF(mag_dust, boxsize, nbins=30, useMask=False)
        bins_nd, logphi_nd, logphi_nd_err = cal_UVLF(mag_nodust, boxsize, nbins=30, useMask=False)

        if combined_bins is None:
            combined_bins = bins  # all boxes share the same bins
    ...
    # ---- COMBINE (take max) ----
    combined_dust = np.maximum(dust_25, dust_50)
    combined_nodust = np.maximum(nd_25, nd_50)
    mask = combined_dust > -np.inf
    mask_nd = combined_nodust > -np.inf

    combined_dust = combined_dust[mask]
    combined_nodust = combined_nodust[mask_nd]
    bins = bins[mask]
    bins_nd = bins_nd[mask_nd]
    # ---- plot combined dust ----
    ax.plot(
        bins, combined_dust,
        color="green",
        linewidth=2.0,
        label="Combined (dust)" if i == 0 else ""
    )

    # ---- plot combined no-dust ----
    ax.plot(
        bins_nd, combined_nodust,
        color="green",
        linestyle="--",
        linewidth=2.0,
        label="Combined (no dust)" if i == 0 else ""
    )

    # ---- redshift label ----
    z = int(round(obj.simulation.redshift))
    ax.text(0.05, 0.9, f"z = {z}", transform=ax.transAxes,
            fontsize=12, verticalalignment='top', color='black')

    ax.grid(alpha=1.0)

# labels on outer axes
for ax in axes[4:]:
